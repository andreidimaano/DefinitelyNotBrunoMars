...
def audio_to_spectrogram(src_dir, artist, audioAug=None, augment_type=None, audioFac=None):
    """
    ARGS
        ...
        for file_name in tqdm(files):
            if file_name.endswith('.flac') or file_name.endswith('.mp3') or file_name.endswith('.wav'):
                try:
                    ...
                    if audioAug:
                        for i,audioAugFunc in enumerate(audioAug): 
                            fac = audioFac if not augment_type else audioFac[i]
                            audio = audioAugFunc(data=audio, factor=fac)
